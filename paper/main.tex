...
\title{Harness Amplification on SWE-bench Pro:\\
...
\begin{abstract}
How much can a harness amplify an accessible open-weight model on a public repository-repair benchmark? This report studies OpenCodeOrchestra (OCO), an opencode-based Project Manager $\rightarrow$ Orchestrator $\rightarrow$ specialist-agent workflow, on SWE-bench Pro with Qwen3.6-27B. In a seeded-subset pilot, OCO resolved 162/223 submitted patch rows: 72.6\% strict (Wilson 95\% CI 66.4--78.1\%). The result is not a leaderboard claim; it is evidence that harness design can materially change what a not-frontier model achieves. A later full-run deployment collapse is used as a systems lesson: harness, controller, and serving choices must be evaluated at task level, not only by throughput.
\end{abstract}
...
The artifact trail shows (i) a strong seeded-subset pilot under an archived B200 non-speculative profile, (ii) transparent denominator accounting, and (iii) a later deployment-profile collapse caused by worse generated patches rather than evaluator failure. The collapse is part of the harness story: if the harness is the treatment, serving and controller changes are treatment changes too.
...
\begin{enumerate}
  \item A deterministic, repository-balanced seeded-subset pilot testing how far an OCO-style harness can push accessible Qwen3.6-27B on SWE-bench Pro.
  \item A measured OCO harness configuration where PM-to-Orchestrator delegation was broadly observed and Auditor participation was available but not consistently enforced.
  ...
  \item A deployment-profile failure showing that harness changes must be judged by task outcomes, not only by token-throughput or patch-generation completeness.
\end{enumerate}
...
\node[font=\footnotesize, text=ocoMute, below=0.65cm of $(inv)!0.5!(aud)$] {specialists are called on demand; they support the main flow rather than owning it};
\end{tikzpicture}
...
\section{Harness Sensitivity: Deployment-Profile Collapse}
...
Five old-pass/current-regressed witnesses were inspected patch-by-patch. The current patches failed by ordinary code-quality mechanisms: Flipt had incompatible config/exporter symbols; Navidrome left an expected helper undefined; Vuls missed macOS constants and scanner wiring; Teleport invented a different matcher API; and OpenLibrary changed language serialization into the wrong representation. The H200 FP8 + MTP-4 run is a failed deployment-profile experiment. The actionable lesson for harness research is direct: measure the workflow at task level before trusting any change that makes the agent faster, cheaper, or easier to run.
...
The positive pilot lacks a same-model, same-serving mini-SWE-agent or SWE-agent baseline on the exact seeded subset. It is strongest as an engineering pilot and artifact-backed case study. Follow-up studies should run matched baselines and serving-profile A/B tests when budget or compute is available.
...
\paragraph{Future harness ablations.}
The next study should isolate the harness contribution directly when compute is available. Natural variants are: Orchestrator-only repair; PM $\rightarrow$ Orchestrator without an Auditor; PM $\rightarrow$ Orchestrator with a strict required Auditor pass; and PM-edit plus Auditor-review, which approximates the PM-edit-heavy paths observed in this pilot. These ablations would turn the current engineering case study into a cleaner measurement of which harness roles produce the gain.
...
OCO's seeded-subset pilot shows that a durable orchestration harness can produce strong repository-repair results with accessible Qwen3.6-27B under an archived B200 non-speculative profile. The larger research question is harness amplification: how much performance comes from the model, and how much comes from the workflow wrapped around it? The failed H200 full run makes the systems lesson sharper: harness design, serving profile, and evaluator evidence are first-class experimental variables in coding-agent research.
