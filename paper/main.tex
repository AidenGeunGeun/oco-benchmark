...
\begin{tikzpicture}[
  ...
  specialist/.style={draw, rounded corners=3pt, minimum width=3.0cm, minimum height=1.0cm, align=center, font=\small, fill=ocoGray, draw=ocoDark!55, line width=0.6pt},
  flow/.style={-Latex, line width=0.9pt, draw=ocoDark!75},
  support/.style={-Latex, line width=0.6pt, draw=ocoMute, dashed}
]
...
\draw[support] (pm.south) to[bend right=15] (inv.north);
\draw[support] (orch.south) to[bend right=15] (aud.north);
\draw[support] (inv.east) to[bend right=10] (orch.south west);
\end{tikzpicture}
\caption{OCO workflow. PM owns task framing; Orchestrator owns implementation. Specialists are pulled in on demand (dashed) and return evidence or review back into the main flow.}
\label{fig:arch}
\end{figure}
...
\begin{tikzpicture}[
  ...
  stage/.style={draw, rounded corners=3pt, minimum height=1.25cm, minimum width=2.8cm, align=center, font=\small, line width=0.6pt},
  source/.style={stage, fill=ocoBlue!12, draw=ocoBlue!75!black},
  subset/.style={stage, fill=ocoTeal!18, draw=ocoTeal!80!black},
  bundle/.style={stage, fill=ocoOrange!18, draw=ocoOrange!80!black},
  result/.style={stage, fill=ocoGray, draw=ocoDark!55},
  flow/.style={-Latex, line width=0.9pt, draw=ocoDark!75}
]
\node[source] (a) {Public Pro\\\footnotesize 731 rows};
\node[subset, right=of a] (b) {Seeded 250\\\footnotesize subset};
\node[bundle, right=of b] (c) {223 submitted\\\footnotesize clean patches};
\node[result, right=of c] (d) {Replay\\\footnotesize 162 / 43 / 18};
\draw[flow] (a) -- (b);
\draw[flow] (b) -- (c);
\draw[flow] (c) -- (d);
\end{tikzpicture}
...
\begin{axis}[
  ...
  bar width=11pt,
  xmin=0, xmax=100,
  xlabel={\small Pilot pass rate (\%) with Wilson 95\% CI},
  symbolic y coords={NodeBB,Tutanota,Qutebrowser,Flipt,Element,Navidrome,Teleport,OpenLibrary,Ansible,Vuls,ProtonMail},
  ytick=data,
  yticklabels={%
    NodeBB \scriptsize 39.1\% (9/23),%
    Tutanota \scriptsize 66.7\% (10/15),%
    Qutebrowser \scriptsize 66.7\% (14/21),%
    Flipt \scriptsize 71.4\% (15/21),%
    Element \scriptsize 72.7\% (16/22),%
    Navidrome \scriptsize 76.2\% (16/21),%
    Teleport \scriptsize 76.2\% (16/21),%
    OpenLibrary \scriptsize 81.8\% (18/22),%
    Ansible \scriptsize 83.3\% (15/18),%
    Vuls \scriptsize 84.2\% (16/19),%
    ProtonMail \scriptsize 85.0\% (17/20)},
  yticklabel style={align=right, font=\small},
  axis y line*=left,
  axis x line*=bottom,
  xtick={0,20,40,60,80,100},
  enlarge y limits=0.06,
  major grid style={line width=0.1pt,draw=gray!25},
  xmajorgrids,
  tick align=outside,
...
\addplot+[
  fill=ocoTeal!60,
  draw=ocoTeal!90!black,
  line width=0.4pt,
  error bars/.cd,
  x dir=both, x explicit,
  error bar style={black, line width=0.55pt},
  error mark options={mark size=2pt, line width=0.55pt}
] coordinates {
  (39.1,NodeBB) +- (16.9,20.1)
  (66.7,Tutanota) +- (25.0,18.1)
  (66.7,Qutebrowser) +- (21.3,16.1)
  (71.4,Flipt) +- (21.4,14.8)
  (72.7,Element) +- (20.8,14.2)
  (76.2,Navidrome) +- (21.3,13.2)
  (76.2,Teleport) +- (21.3,13.2)
  (81.8,OpenLibrary) +- (20.3,10.9)
  (83.3,Ansible) +- (22.5,10.9)
  (84.2,Vuls) +- (21.8,10.3)
  (85.0,ProtonMail) +- (21.0,9.8)
};
\draw[dashed, ocoDark!70, line width=0.6pt] (axis cs:72.6,NodeBB) -- (axis cs:72.6,ProtonMail);
\node[font=\scriptsize\itshape, text=ocoDark!75, anchor=south west] at (axis cs:73,ProtonMail) {overall 72.6\%};
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
  ...
  bar width=22pt,
  xmin=0, xmax=110,
  xlabel={\small Strict pass rate (\%) through the same Modal/SWE-bench Pro evaluator},
  symbolic y coords={h200,b200},
  ytick={h200,b200},
  yticklabels={{H200/FP8/MTP-4 (208/731)},{B200/FP8/non-MTP (162/223)}},
  yticklabel style={align=right, font=\small},
  nodes near coords={\pgfmathprintnumber[fixed,precision=1,zerofill]{\pgfplotspointmeta}\%},
  nodes near coords align={horizontal},
  every node near coord/.append style={xshift=24pt, font=\small\bfseries, text=black},
  axis y line*=left,
  axis x line*=bottom,
  xtick={0,20,40,60,80,100},
...
\addplot[
  xbar,
  draw=ocoOrange!85!black,
  line width=0.4pt,
  fill=ocoOrange!65,
  error bars/.cd,
  x dir=both, x explicit,
  error bar style={black, line width=0.7pt},
  error mark options={mark size=3pt, line width=0.7pt}
] coordinates {
  (28.45,h200) +- (3.15,3.38)
};
\addplot[
  xbar,
  draw=ocoTeal!90!black,
