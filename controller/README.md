...
For real runs, the controller reads the production OCO config directory, writes a fresh `oco-config-snapshot/` under the run directory, and points the subprocess at an isolated per-attempt home. It copies the primary agent plus `orchestrator`, `investigator`, `auditor`, and `compaction`; it strips `general`, `explore`, `test_runner`, `web-search`, `docs`, plugins, MCP servers, and skills. The stripped native subagents are emitted with `disable: true` in the snapshot so OCO's built-ins are removed from the resolved agent registry. The snapshot enables autocompaction with `auto=true`, `prune=true`, `reserved=20000`, and the compaction agent model set to the benchmark self-host model name.

For Qwen-family model names, the materializer copies `prompts.qwen/pm.txt` and `prompts.qwen/orchestrator.txt` into the snapshot instead of the production PM and Orchestrator prompt files. This is a documented per-model adaptation for the pod run; non-Qwen snapshots keep production prompt bytes unchanged. The manifest records the selected prompt variant and overlay source paths.

The materializer writes `strip-diff-manifest.json` beside the snapshot so reviewers can see what was kept, removed, disabled, and overridden. The snapshot is not redacted; it stays on the operator machine and inference pod, which already hold the same benchmark secrets. If production config is malformed, the materializer removes any partial snapshot and writes `oco-config-materialization-error.json` with the reason.
...
Real runs enable the production-fidelity boundary monitor. It snapshots protected roots, normally the OCO source checkout and production OCO config, plus monitored parent roots used to catch writes outside the allowed run directory. The configured repo cache directory is also treated as an allowed root because the controller owns those setup writes. On systems with `strace`, the real adapter records write-like filesystem syscalls in `filesystem-trace.log`; the classifier ignores failed write-like syscalls and resolves relative `*at` paths through known parent file descriptors before deciding whether a write is outside allowed roots. Real-run proofs require the trace so unexpected writes outside the monitored roots are still visible. At attempt end it writes `boundary-proof.md`; protected-root changes, monitored writes outside allowed/protected roots, missing required trace, or traced out-of-bounds writes mark the proof failed, while run-directory and repo-cache changes are listed as allowed.
